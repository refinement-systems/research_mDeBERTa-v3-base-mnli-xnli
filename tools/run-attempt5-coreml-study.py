...
DEFAULT_SCRATCHPAD_ROOT = REPO_ROOT / "scratchpad" / "attempt5_coreml_focus"
DEFAULT_CATALOG_PATH = REPO_ROOT / "research" / "attempt5_coreml-focus" / "study_quantization_catalog.json"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--force-datasets",
        action="store_true",
        help="Regenerate the reused attempt4 evaluation pack under the attempt5 scratchpad.",
    )
    parser.add_argument(
        "--skip-controls",
        action="store_true",
...
def benchmark_coreml(
    ...
    *,
    ...
    if not quantizations:
        raise RuntimeError(f"No CoreML benchmark candidates are available for mode={mode}")
...
def build_report(
    ...
    *,
    ...
    command = [
        sys.executable,
        "-u",
        str(REPO_ROOT / "tools" / "build-attempt5-coreml-report.py"),
        "--datasets-manifest",
...
def main() -> int:
    ...
    report_output_prefix = reports_root / "attempt5-coreml-summary"
    control_quantizations = [] if args.skip_controls else list(CONTROL_QUANTIZATIONS)
    control_failures: dict[str, str] = {}

    print("Running CoreML reference and fp16 primary candidates.")
    for quantization in PRIMARY_QUANTIZATIONS:
        ...
        for quantization in control_quantizations:
            ...
            for dataset_name in [*dataset_manifest["smoke_datasets"], *dataset_manifest["validation_datasets"]]:
                if not run_study(
                    ...
                    control_failures[quantization] = dataset_name
                    failed = True
                    break
            if failed:
                print(
                    f"Control {quantization} failed on {control_failures[quantization]}; "
                    "continuing because controls are secondary."
                )

    validation_summary_path = summarize_role(
        scratchpad_root,
        "fidelity_validation",
        reports_root / "attempt5-validation-summary",
    )
    ...
    if not validation_complete:
        raise RuntimeError(
            "No CoreML candidates completed the validation pack. "
            "Inspect attempt5-validation-summary.json and the study logs."
        )

    print("Benchmarking complete CoreML validation candidates.")
    validation_runtime_csv_path = benchmark_coreml(
        scratchpad_root,
        executable,
        persistent_executable,
        mode="persistent",
        quantizations=validation_complete,
        output_prefix=reports_root / "attempt5-validation-coreml-persistent",
    )
    ...
    if not locked_quantizations:
        raise RuntimeError(
            "No CoreML candidates survived the development gates. "
            "Inspect attempt5-coreml-summary.json for the rejection reasons."
        )

    manifest: dict[str, object] = {
        "scratchpad_root": str(scratchpad_root),
        "catalog": str(catalog_path),
        "control_quantizations_requested": control_quantizations,
    ...
    if not args.skip_test:
        print("Running locked CoreML final-test and stress-test evaluation.")
        ...
        test_summary_path = summarize_role(
            scratchpad_root,
            "fidelity_test",
            reports_root / "attempt5-test-summary",
        )
        stress_summary_path = summarize_role(
            scratchpad_root,
            "stress_test",
            reports_root / "attempt5-stress-summary",
        )
        cold_benchmark_csv_path = benchmark_coreml(
            scratchpad_root,
            executable,
            persistent_executable,
            mode="coldstart",
            quantizations=locked_quantizations,
            output_prefix=reports_root / "attempt5-test-coreml-cold",
        )
    ...
    manifest_path = reports_root / "attempt5-manifest.json"
