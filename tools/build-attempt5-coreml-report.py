...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build the attempt5 CoreML report by joining study summaries, "
            "development gates, and CoreML runtime/RSS benchmarks."
        )
    )
...
def validation_gate(
    ...
    if candidate_runtime is None:
        reasons.append("missing CoreML persistent benchmark row")
    if reference_runtime is None:
        reasons.append("missing reference CoreML persistent benchmark row")
    ...
    if candidate_runtime is not None and reference_runtime is not None:
        ...
        if candidate_peak_rss is None or reference_peak_rss is None:
            reasons.append("missing peak RSS for CoreML persistent benchmark")
        else:
...
def dominates_final(left: dict[str, Any], right: dict[str, Any]) -> bool:
    ...
    left_values = (
        left.get("size_bytes"),
        left.get("coreml_persistent_warm_median_ms"),
        left.get("coreml_persistent_resident_after_warmup_bytes"),
    )
    right_values = (
        right.get("size_bytes"),
        right.get("coreml_persistent_warm_median_ms"),
        right.get("coreml_persistent_resident_after_warmup_bytes"),
    )
...
def choose_recommendation(frontier_rows: list[dict[str, Any]]) -> dict[str, Any] | None:
    ...
    chosen = min(
        ...
        key=lambda row: (
            ...
            float(row.get("coreml_persistent_warm_median_ms") or float("inf")),
            float(row.get("coreml_persistent_resident_after_warmup_bytes") or float("inf")),
        ...
        rows.append(
            {
                ...
                "coreml_persistent_load_median_ms": optional_float(runtime, "load_median_ms"),
                "coreml_persistent_warm_median_ms": optional_float(runtime, "warm_median_ms"),
                ...
                "coreml_cold_load_median_ms": optional_float(cold, "load_median_ms"),
                "coreml_cold_warm_median_ms": optional_float(cold, "warm_median_ms"),
            }
        )
    return rows
...
def main() -> int:
    ...
    lines = [
        "# Attempt5 CoreML Deployment Study",
        "",
        ...
        "## Candidate Summary",
        "",
        "| Candidate | Gate | Dev Acc | Dev Float Agree | CoreML Warm | CoreML Steady RSS | CoreML Peak RSS | Cold Load |",
        "| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: |",
    ]
    for row in candidate_summary_rows:
        ...
        lines.append(
            ...
            f"{ms_text(row.get('coreml_persistent_warm_median_ms'))} | "
            f"{mib_text(row.get('coreml_persistent_resident_after_warmup_bytes'))} | "
            f"{mib_text(row.get('coreml_persistent_peak_rss_bytes'))} | "
            f"{ms_text(row.get('coreml_cold_load_median_ms'))} |"
        )
    ...
    if test_aggregates:
        lines.extend(
            [
                ...
                "",
                "| Candidate | Size | Final Acc | Final Float Agree | CoreML Warm | CoreML Steady RSS | Frontier |",
                "| --- | ---: | ---: | ---: | ---: | ---: | --- |",
            ]
        )
        for row in candidate_summary_rows:
