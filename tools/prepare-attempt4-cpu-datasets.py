...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Prepare the attempt4 CPU dataset pack with frozen calibration/smoke inputs plus "
            "full development, locked-final, and stress-test evaluation datasets."
        )
    )
    parser.add_argument(
        "--scratchpad-root",
    ...
    parser.add_argument(
        "--anli-dataset",
        default="facebook/anli",
        help="ANLI dataset id (default: facebook/anli)",
    )
    parser.add_argument(
        "--wanli-dataset",
        default="alisawuffles/WANLI",
        help="WANLI dataset id (default: alisawuffles/WANLI)",
    )
    parser.add_argument(
        "--hans-dataset",
        default="jhu-cogsci/hans",
        help="HANS dataset id (default: jhu-cogsci/hans)",
    )
...
def resolve_xnli_languages(args: argparse.Namespace) -> tuple[str, ...]:
    return tuple(args.xnli_languages or DEFAULT_XNLI_LANGUAGES)


def build_targets(args: argparse.Namespace) -> dict[str, list[ExportTarget]]:
    validation_targets = [
        ExportTarget(
...
def main() -> int:
    ...
    copied_calibration = copy_frozen_datasets(dataset_root, args.force, CALIBRATION_DATASET_FILENAMES)
    copied_smoke = copy_frozen_datasets(dataset_root, args.force, SMOKE_DATASET_FILENAMES)
    ...
    xnli_languages = resolve_xnli_languages(args)
